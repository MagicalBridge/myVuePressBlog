...
总结来说，useRef返回值的改变并不会造成页面更新。而且useRef类似于react中的全局变量并不存在不同次render中的`state/props`的作用域隔离机制。这就是useRef和useState这两个hook主要的区别。
...
我们可以将useRef返回值看作一个组件内部全局共享变量，它会在渲染内部共享一个相同的值。相对`state/props`他们是独立于不同次render中的内部作用域值。
